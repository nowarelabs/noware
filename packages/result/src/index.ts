...
function err<TagValue extends string | number | boolean>(
  ...
  if (isTaggedData) {
    ...
    const errorMessage =
      ...
      (typeof tagValue === "string" || typeof tagValue === "number" || typeof tagValue === "boolean"
        ? String(tagValue)
        : "Error");
...
type MatchHandlers<T, R, TagName extends string = "tag"> = 0 extends 1 & T
  ? {
  ...
  : {
      [K in Extract<
        T extends Record<TagName, string | number | boolean> ? T[TagName] : never,
        PropertyKey
      >]?: (payload: Omit<Extract<T, Record<TagName, K>>, TagName>) => R;
    } & {
      error?: (error: ErrorInfo) => R;
      default?: (payload: unknown) => R;
      [key: string]: any;
...
>(
  ...
  if (!result.success) {
    ...
    if (result.details && typeof result.details === "object") {
      const tagValue = (result.details as Record<string, unknown>)[tagName] as
        | (T extends Record<TagName, any> ? T[TagName] : any)
        | undefined;
  ...
  const tagValue = data[tagName] as (T extends Record<TagName, any> ? T[TagName] : any) | undefined;
...
function isTagged<Tag extends string>(
  value: unknown,
  tag: Tag,
): value is { [K in Tag]: string | number | boolean };
