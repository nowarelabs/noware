...
function err(errorOrTagged, message = "Error", status = 500, code = "ERROR", stack, detailsOrTagName, tagName = "tag") {
    // Check if first argument is tagged data
    const actualTagName = (typeof detailsOrTagName === "string" ? detailsOrTagName : tagName) || tagName;
    const isTaggedData = typeof errorOrTagged === "object" &&
        errorOrTagged !== null &&
    ...
    if (isTaggedData) {
        ...
        const errorMessage = (typeof taggedData.error === "string" ? taggedData.error : null) ||
...
const safe = (fn) => {
    try {
        ...
        return err(error.message || "Operation failed", "Operation failed", 500, "ERROR", stack, details);
    }
};
const safeAsync = async (fn) => {
    try {
        ...
        const stack = error.stack;
        const details = getErrorDetails(e);
        return err(error.message || "Operation failed", "Operation failed", 500, "ASYNC_ERROR", stack, details);
    }
};
function when(condition, value, defaultValue) {
    return condition ? value : defaultValue;
}
const whenResult = (condition, value, errorMessage = "Condition not met", errorCode = "CONDITION_FAILED", statusCode = 400) => (condition ? ok(value) : err(errorMessage, errorMessage, statusCode, errorCode));
...
const matchCodeResult = (code, overrides, errorMessage = `No match found for code: ${code}`, errorCode = "CODE_NOT_FOUND", statusCode = 404) => {
...
const matchStatus = (status, overrides, defaultValue) => (status in overrides ? overrides[status] : defaultValue);
const matchStatusResult = (status, overrides, errorMessage = `No match found for status: ${status}`, errorCode = "STATUS_NOT_FOUND", statusCode = 404) => {
...
const match = (result, handlers, tagName = "tag") => {
    ...
    return err(`No handler found for tag value: ${String(tagValue)}`, "Match error", 500, "MATCH_ERROR");
};
...
export async function tryN(n, fn, options) {
    ...
    n = Math.floor(n);
    return await tryWhile(fn, (err, nextAttempt) => {
