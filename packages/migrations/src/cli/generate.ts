...
export const generateCommand = defineCommand({
  ...
  args: {
    ...
    populateFrom: {
      type: "string",
      description:
        "Comma-separated list of tables this DO populates from (e.g., tournaments,brackets,matches)",
    },
    doClass: {
      type: "string",
  ...
  async run({ args }) {
    ...
    if (isDO) {
      ...
      consola.info(
        `Edit the file to add columns, then run 'pnpm db:scaffold --all' to generate the full project.`,
      );
    } else {
      consola.info(
        `Edit the file to add columns, then run 'pnpm db:scaffold --all' to generate the full project.`,
