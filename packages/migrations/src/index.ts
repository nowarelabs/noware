...
export class TableBuilder {
  ...
  decimal(
    name: string,
    options: ColumnOptions & { precision?: number; scale?: number } = {},
  ) {
  ...
  foreignKey(
    ...
    this.foreignKeys.push(
      new ForeignKey(columnName, toTable, toColumn, options),
    );
    return this;
  }
  ...
  index(
    ...
    this.indexes.push(
      new Index(Array.isArray(columns) ? columns : [columns], options),
    );
    return this;
  }
  ...
  hasMany(
    targetTable: string,
    options: RelationshipOptions & { name?: string } = {},
  ) {
...
export abstract class Migration implements TableAltererMigration {
  ...
  public async createTable(
    ...
    const callback =
      typeof arg2 === "function" ? arg2 : (arg3 as (t: TableBuilder) => void);
  ...
  protected createJoinTable(
    table1: string,
    table2: string,
    options: JoinTableOptions = {},
  ) {
    const name =
      options.tableName || `${table1}_${table2}`.split("_").sort().join("_");
