...
export const STANDARD_HANDLERS: Record<string, HandlerFunc> = {
      ...
      (() => {
        ...
        table.columns.forEach((col: Column) => {
          ...
          const colParts = sql.composite(
            ...
            col.options.primaryKey
              ? sql.composite(sql.op(" "), sql.primaryKey())
              : sql.raw(""),
            col.options.primaryKey &&
              col.options.autoincrement &&
              strategy.dialect === "sqlite"
              ? sql.composite(sql.op(" "), sql.key("AUTOINCREMENT"))
              : sql.raw(""),
            col.options.notNull
              ? sql.composite(sql.op(" "), sql.key("NOT NULL"))
              : sql.raw(""),
            col.options.unique
              ? sql.composite(sql.op(" "), sql.key("UNIQUE"))
              : sql.raw(""),
          );
          ...
          if (col.options.check) {
            ...
            if (
              typeof col.options.check === "object" &&
              "__isSql" in col.options.check
            ) {
              colParts.append(
                sql.raw((col.options.check as SqlExpression).sql),
              );
            } else {
        ...
        if (table.primaryKey) {
          const pks = Array.isArray(table.primaryKey)
            ? table.primaryKey
            : [table.primaryKey];
        ...
        table.constraints?.forEach((con: Constraint) => {
          ...
          if (con.type === "unique") {
            ...
            if (
              typeof con.definition === "object" &&
              "__isSql" in (con.definition as any)
            ) {
        ...
        table.indexes.forEach((idx) => {
          const idxName =
            idx.name || `idx_${name}_${idx.columns.join("_")}`;
  ...
  removeIndex: (action) => {
    ...
    const idxName = options.name || `idx_${tableName}_${columns.join("_")}`;
    return ok(
      sql.statement().append(sql.key("DROP INDEX IF EXISTS "), sql.id(idxName)),
    );
  },
  ...
  changeColumnDefault: (action) => {
    console.warn(
      `ALTER TABLE ALTER COLUMN SET DEFAULT is not supported in SQLite.`,
    );
    return ok(null);
  },
  ...
  changeColumnNull: (action) => {
    console.warn(
      `ALTER TABLE ALTER COLUMN SET NOT NULL is not supported in SQLite.`,
    );
    return ok(null);
  },
  ...
  recreateTable: (action, strategy) =>
    ok(
      (() => {
        ...
        const createStmtRes = STANDARD_HANDLERS.createTable(
          { name, table },
          strategy,
        );
    ...
    ok(
      (() => {
        ...
        const stmt = sql
          .statement()
          .append(
            sql.key("CREATE VIEW IF NOT EXISTS "),
            sql.id(name),
            sql.key(" AS "),
          );
        if (
