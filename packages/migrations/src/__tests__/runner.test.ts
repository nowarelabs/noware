...
describe("MigrationRunner", () => {
  ...
  it("runs pending migrations up", async () => {
    ...
    expect(mockDb.run).toHaveBeenCalledWith(
      stringWith("INSERT INTO schema_migrations"),
    );
  ...
  it("only runs pending migrations", async () => {
    ...
    expect(mockDb.run).not.toHaveBeenCalledWith(
      expect.stringContaining("20260101000000"),
    );
    // Wait, let's be more specific
    const insertCalls = mockDb.run.mock.calls.filter((c: any) =>
      c[0]?.sql?.includes("INSERT"),
    );
