...
describe("Advanced SQLite Features", () => {
  ...
  it("generates STRICT and WITHOUT ROWID tables", () => {
    ...
    const res = STANDARD_HANDLERS.createTable(
      { name: "test_table", table: t },
      strategy,
    );
  ...
  it("generates virtual tables (FTS5)", () => {
    ...
    const res = STANDARD_HANDLERS.createVirtualTable(
      { name: "search_idx", builder: v },
      strategy,
    );
    expect(res.success).toBe(true);
    if (!res.success) throw new Error("Result failed");
  ...
  it("generates JSON functions", () => {
    ...
    expect(extract.toSql(strategy).data?.value).toBe(
