...
import { RouteDrawer, Router } from "./index";
...
describe("Router", () => {
  ...
  describe("RouteDrawer Versioning", () => {
    it("should correctly prefix routes with version", async () => {
      const router = new Router();
      ...
      const res2 = await router.handle(new Request("http://localhost/v2/test"), {}, mockCtx);
      expect(await res2.text()).toBe("v2 ok");
    });
  });
});
