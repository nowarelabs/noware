...
> extends BaseRpcTarget {
  ...
  async list(params: Record<string, unknown> = {}): Promise<any> {
    ...
    try {
      ...
      this.logger.debug(
        `[LIST] count=${Array.isArray(jsonResult) ? jsonResult.length : "unknown"}`,
      );
      return jsonResult;
    } catch (error) {
