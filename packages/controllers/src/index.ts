...
export abstract class BaseController<
  ...
  Service = unknown,
> {
  static beforeActions: HookConfig<
    BaseController<unknown, unknown, unknown>,
    unknown,
    unknown
  >[] = [];
  static afterActions: HookConfig<
    BaseController<unknown, unknown, unknown>,
    unknown,
    unknown
  >[] = [];
  ...
  protected get params(): Record<string, unknown> {
    ...
    const wrappedKey = this.controller_name
      .toLowerCase()
      .replace("controller", "");
  ...
  async render(options: {
    ...
    if (view) {
      const LayoutClass =
        options.layout ||
        (this.constructor as unknown as Record<string, unknown>).layout;
  ...
  protected validate<V>(
    Validator: Constructor<IValidator<V>>,
    data: unknown,
  ): V {
    return new Validator(data).validate();
  }

  protected normalize<N>(
    Normalizer: Constructor<INormalizer<N>>,
    data: unknown,
  ): N {
  ...
  static action<
    ...
    Service = unknown,
  >(
    this: new (req: Request, env: Env, ctx: RouterContext<Env, Ctx>) => T,
    name: keyof T,
  ) {
    return async (
      req: Request,
      ...
      for (const hook of ctor.beforeActions) {
        if (shouldRunHook(hook, name)) {
          ...
          if (hook.normalize) {
            ...
            const dataToNormalize = Object.keys(this._paramsOverride).length > 0 
              ? this._paramsOverride 
              : (json || {});
            if (dataToNormalize && Object.keys(dataToNormalize).length > 0) {
              (this.ctx as Record<string, unknown>).validJson =
                this.normalize(hook.normalize, dataToNormalize);
            }
          }
          ...
          if (hook.validate) {
            ...
            try {
              ...
              ctxWithJson.validJson = this.validate(
                hook.validate,
                ctxWithJson.validJson,
              );
            } catch (err: unknown) {
          ...
          if (hook.run) {
            if (typeof hook.run === "function") {
              ...
              const hookFn = (
                this as unknown as Record<string | symbol, Function>
              )[actionName];
...
> extends BaseController<Env, Ctx, Service> {
  protected abstract getModel(): TModel;
	...
	protected async respondWith(data: any, options: { status?: number } = {}): Promise<Response> {
		...
		const wantsHtmlOnly = accept.includes("text/html") && !accept.includes("application/json") && !accept.includes("*/*");
		const wantsXml = accept.includes("application/xml");
		const wantsCsv = accept.includes("text/csv");
		const wantsXlsx = accept.includes("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
	...
	async pluck(): Promise<Response> {
		const data = (await this.getRequestData()) as { column?: string; conditions?: Record<string, any> };
	...
	async listChildIds(): Promise<Response> {
		...
		this.logger.info(`[LIST_CHILD_IDS] ${this.controller_name}#${identifier}`, { relation, count: ids.length });
	...
	async listParentIds(): Promise<Response> {
		...
		this.logger.info(`[LIST_PARENT_IDS] ${this.controller_name}#${identifier}`, { relation, count: ids.length });
		return this.json({ ids });
	}
	...
	async listSiblingIds(): Promise<Response> {
		...
		this.logger.info(`[LIST_SIBLING_IDS] ${this.controller_name}#${identifier}`, { relation, count: ids.length });
		return this.json({ ids });
	}
	...
	async listCousinIds(): Promise<Response> {
		...
		this.logger.info(`[LIST_COUSIN_IDS] ${this.controller_name}#${identifier}`, { relation, count: ids.length });
		return this.json({ ids });
	}
	...
	async listAncestorIds(): Promise<Response> {
		...
		this.logger.info(`[LIST_ANCESTOR_IDS] ${this.controller_name}#${identifier}`, { relation, count: ids.length });
		return this.json({ ids });
	}
	...
	async listDescendantIds(): Promise<Response> {
		...
		this.logger.info(`[LIST_DESCENDANT_IDS] ${this.controller_name}#${identifier}`, { relation, count: ids.length });
		return this.json({ ids });
	}

	async listAssociatedThroughIds(): Promise<Response> {
		const { relation, through, id } = await this.getRequestData() as { relation?: string; through?: string; id?: string };
		const identifier = id || this.getIdentifier();
		...
		this.logger.debug(`[LIST_ASSOCIATED_THROUGH_IDS] ${this.controller_name}#${identifier}`, { relation, through });
		
		const model = this.getModel() as any;
		const ids = await model.listAssociatedThroughIds(relation, through, identifier);
		
		this.logger.info(`[LIST_ASSOCIATED_THROUGH_IDS] ${this.controller_name}#${identifier}`, { relation, through, count: ids.length });
		return this.json({ ids });
	}
	...
	async listRelatedIds(): Promise<Response> {
		...
		this.logger.info(`[LIST_RELATED_IDS] ${this.controller_name}#${identifier}`, { relation, count: ids.length });
		return this.json({ ids });
	}
