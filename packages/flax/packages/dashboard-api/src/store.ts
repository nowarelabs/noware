...
export async function openStage(
  ...
  const entryDetail = returned ? `↩ returned` : (reentered?.n ?? 0) > 0 ? `↩ returned` : detail ?? null;
  await db
    .prepare(
...
export async function listStages(
  ...
  const { results } = await db
    .prepare(
      `SELECT * FROM flax_stages WHERE conversation_id = ? ORDER BY entered_at ASC, id ASC`,
    )
    .bind(conversationId).all<StageRow>();
  return results ?? [];
}
...
export async function upsertInstanceMeta(
  ...
  if (meta.title !== undefined) { sets.push('title = ?'); values.push(meta.title); }
  if (meta.origin !== undefined) { sets.push('origin = ?'); values.push(meta.origin); }
  if (meta.currentStage !== undefined) { sets.push('current_stage = ?'); values.push(meta.currentStage); }
  if (meta.currentAgent !== undefined) { sets.push('current_agent = ?'); values.push(meta.currentAgent); }
  if (meta.status !== undefined) { sets.push('status = ?'); values.push(meta.status); }
  if (meta.lastActivityAt !== undefined) { sets.push('last_activity_at = ?'); values.push(meta.lastActivityAt); }
...
export async function upsertHitl(db: D1Database, hitl: {
  id: string;
...
export async function listHitl(
  db: D1Database,
  conversationId: string,
): Promise<HitlRow[]> {
  const { results } = await db
    .prepare(
      `SELECT * FROM flax_hitl WHERE conversation_id = ? ORDER BY created_at ASC, id ASC`,
    )
    .bind(conversationId).all<HitlRow>();
  return results ?? [];
}
...
export async function upsertArtifact(db: D1Database, artifact: {
  conversation_id: string;
  stage?: string | null;
  agent?: string | null;
  type: string;
...
export async function listArtifacts(
  ...
  const { results } = await db
    ...
    .bind(conversationId).all<ArtifactRow>();
  return results ?? [];
}
