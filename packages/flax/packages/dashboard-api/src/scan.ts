...
function isDispatchCall(part: Part): boolean {
  return part.toolName === 'dispatch_agent' && (part.type === 'dynamic-tool' || part.type === 'tool-input');
}

function isHitlCall(part: Part): boolean {
  return part.toolName === 'request_human_input' && (part.type === 'dynamic-tool' || part.type === 'tool-input');
}
...
function classifyUrl(url: string): string | null {
  ...
  if (u.includes('figma.com') || u.includes('miro.com') || u.includes('excalidraw.com')) return 'diagram';
  if (u.includes('github.com') && u.includes('/issues/')) return 'issue';
  if (u.includes('notion.so') || u.includes('confluence') || u.includes('docs.google.com')) return 'doc';
  ...
  for (const message of messages) {
    for (const part of message.parts ?? []) {
      ...
      if (isDispatchCall(part)) {
        ...
        const targetConversation = typeof args.conversationId === 'string' ? args.conversationId : conversationId;
        if (agent) {
          const stage =
            (args.stage as StageId | undefined) ?? (asRecord(args.attributes).stage as StageId | undefined) ?? stageForAgent(agent);
