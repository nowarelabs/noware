 ...
 */
export async function authForRepo(env: { FLAX_DB?: D1Database; [key: string]: unknown }, repo?: string): Promise<string> {
  const pat = env.GITHUB_TOKEN;
  if (typeof pat === 'string' && pat.length > 0) {
    return `Bearer ${pat}`;
  }
  const db = env.FLAX_DB;
  if (!db) throw new Error('No GitHub credential available: set GITHUB_TOKEN or bind FLAX_DB with a configured App.');
