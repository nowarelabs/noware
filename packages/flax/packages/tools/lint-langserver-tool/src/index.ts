...
function lintSource(language: string, content: string): Diagnostic[] {
  ...
  if (language === "html") {
    ...
    if (!/\blang\s*=/.test(content))
      push("warning", "<html> missing lang attribute", 1, "a11y/html-lang");
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.length > 120)
      push("info", `Line too long (${line.length} > 120 chars)`, n, "style/line-length");
    ...
    if (jsLike) {
      ...
      if (/console\.(log|debug)\(/.test(line))
        push("info", "Debug logging left in code", n, "js/no-console");
      if (/\b==\s*[^=]/.test(line) && !/===/.test(line))
        push("info", "Use === instead of ==", n, "js/eqeqeq");
      if (/\beval\s*\(/.test(line))
        push("error", "Avoid eval() - code injection risk", n, "js/no-eval");
      if (/innerHTML\s*=/.test(line))
        push("warning", "Setting innerHTML can lead to XSS", n, "js/no-inner-html");
    }

    if (isPython) {
      if (/^\s*\t/.test(line))
        push("warning", "Tab used for indentation; prefer spaces", n, "python/indent");
      if (/^\s*print\s*\(/.test(line))
        push("info", "print() found; consider logging", n, "python/no-print");
      if (/^\s*except\s*:/.test(line))
        push("warning", "Bare except catches everything", n, "python/bare-except");
      if (/\bimport\s+\*\b/.test(line))
        push("warning", "Star import pollutes the namespace", n, "python/star-import");
    }
  }
  if (jsLike) {
  ...
  async lintFile(input: { filePath: string; language?: string }): Promise<unknown> {
    ...
    if (typeof content !== "string" || content.length === 0) {
      return {
        ...
        language,
        diagnostics: [],
        error: "no file content provided; pass { filePath, content }",
      };
    }
