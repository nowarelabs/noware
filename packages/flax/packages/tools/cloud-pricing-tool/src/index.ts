...
function cheapestFor(
  ...
  if (candidates.length === 0) {
    throw new Error(
      `no instance type satisfies ${vcpus} vCPU / ${memoryGb} GB${gpu ? ` / ${gpu} GPU` : ""}`,
    );
  }
...
export class CloudPricingTool extends WorkerEntrypoint<Env> {
  async estimateCost(input: {
    ...
    provider?: string;
    region?: string;
  }): Promise<{ estimatedMonthlyCost: number }> {
  ...
  async compareInstanceTypes(input: {
    workload: unknown;
    ...
    const rows = input.instanceTypes.map((name) => {
      ...
      return {
        ...
        estimatedMonthlyCost:
          Math.round(instance.ratePerHour * regionMultiplier * count * hours * 100) / 100,
      };
    });
