      ...
      <div className="cf-content conversation">
        <div className="conv-head">
          <div>
            <div className="cf-breadcrumb">
              <span className="crumb-link" onClick={onBack}>Inbox</span>
              <span className="sep">/</span>
              <span className="current mono">{id}</span>
            </div>
            <h2 className="conv-title">{detail?.title ?? 'Conversation'}</h2>
            <div className="conv-meta mono">
              {detail?.status ? <Badge variant={conversationStatusVariant(detail.status)}>{conversationStatusLabel(detail.status)}</Badge> : null}
              {detail?.currentStage ? <span><StageBadge stage={detail.currentStage} /></span> : null}
              {detail?.currentAgent ? <span>{detail.currentAgent}</span> : null}
              <span>{timeAgo(detail?.lastActivityAt)}</span>
            </div>
          </div>
          <div className="actions">
            <Button variant="ghost" size="sm" onClick={onScan} disabled={scanning} title="Re-scan history for stages, HITL and artifacts">
              <Scan size={14} /> {scanning ? 'Scanning…' : 'Scan'}
            </Button>
            <Button variant="ghost" size="sm" onClick={() => void refresh()} title="Refresh history">
        ...
        {detail ? (
          <div className="cf-stats">
            <StatCard
              label="Status"
              value={<Badge variant={conversationStatusVariant(detail.status)}>{conversationStatusLabel(detail.status)}</Badge>}
              hint={working ? 'agent is working' : 'latest run state'}
            />
            <StatCard
              label="Stage"
              value={detail.currentStage ? <StageBadge stage={detail.currentStage} /> : <span className="cf-muted">—</span>}
        ...
        <Tabs
          ...
          tabs={[
            ...
            { value: 'pipeline', label: `Pipeline${detail && detail.stages.length ? ` (${detail.stages.length})` : ''}` },
            { value: 'artifacts', label: `Artifacts${detail && detail.artifacts.length ? ` (${detail.artifacts.length})` : ''}` },
          ]}
          value={activeTab}
          ...
          <div className="cf-panel">
            <div className="pipeline-tab">
              <div className="cf-card rail-card">
                ...
                <PipelineRail stages={detail?.stages ?? []} currentStage={detail?.currentStage ?? null} />
              </div>
                ...
                <div className="cf-card stage-log">
                  <table>
                    ...
                    <tbody>
                      {detail.stages.map((s) => (
                        <tr key={s.id}>
                          <td><StageBadge stage={s.stage} active={s.exited_at === null} /></td>
                          <td><span className="mono">{s.agent}</span></td>
                          <td className="mono">{timeAgo(s.entered_at)}</td>
                          <td className="mono">{s.exited_at ? timeAgo(s.exited_at) : '—'}</td>
                          <td>{s.outcome ? <Badge variant={s.outcome === 'completed' ? 'success' : s.outcome === 'failed' ? 'error' : 'warning'}>{s.outcome}</Badge> : '—'}</td>
                          <td>{s.detail ?? '—'}</td>
                        </tr>
                      ))}
                    </tbody>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <InputGroup>
              ...
              <InputGroup.Addon align="end">
                <InputGroup.Button variant="primary" onClick={() => void onSend()} disabled={!draft.trim()}>
