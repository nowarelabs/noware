...
export function Setup({ onReady }: { onReady: () => void }) {
  ...
  const refresh = () => {
    githubStatus().then(setStatus).catch((err) => setError(err instanceof Error ? err.message : String(err)));
  };
  ...
  const stepOk = (done: boolean) => (
    <span className="step-check" data-ok={done ? 'true' : 'false'}>
      {done ? <CheckCircle size={15} weight="duotone" /> : <span className="num">{!status.configured ? 1 : 2}</span>}
    </span>
  );
      ...
      <div className="setup-hero">
        ...
        <div>
          ...
          <p className="inbox-sub">
            Flax agents open PRs and merge approved work on your repos. Connect a GitHub App so they can act on your behalf.
          </p>
        </div>
        <div style={{ flex: 1 }} />
        {status.configured ? <Badge variant="success">connected</Badge> : <Badge variant="warning">setup required</Badge>}
      </div>
      ...
      <div className="cf-card setup-card">
        <div className="step">
          ...
          <div className="body">
            ...
            <p>
              Opens GitHub's App Manifest flow. The dashboard exchanges the one-time code and stores the App credentials securely in D1.
            </p>
            {status.app ? (
              <div className="done">
                <Badge variant="success">App created</Badge>
                <span className="mono">{status.app.slug} · {status.app.appId}</span>
              </div>
            ) : (
              <Button variant="primary" onClick={() => window.open(manifestUrl, '_blank', 'noopener')}>
        ...
        <div className="step">
          ...
          <div className="body">
            ...
            <p>
              Choose which repositories Flax may act on. After installing, GitHub redirects back and the dashboard verifies the connection.
            </p>
            {status.installation ? (
              <div className="done">
                <Badge variant="success">Installed</Badge>
                <span className="mono">{status.installation.org} · {status.installation.installationId}</span>
              </div>
            ) : status.app ? (
              <Button variant="primary" onClick={() => { if (status.installUrl) window.location.href = status.installUrl; }}>
        ...
        <div className="step">
          ...
          <div className="body">
            ...
            <p>
              A live check confirms Flax can mint installation tokens and reach the GitHub API on your repos.
            </p>
            {status.installation ? (
              <div className="done">
            ...
            {status.configured && status.installation && status.bindingLive ? (
              <Button variant="primary" onClick={onReady}>Continue to dashboard</Button>
            ) : null}
          </div>
        </div>
      </div>
        ...
        {manual ? (
          <div className="manual-form">
            <div className="fields">
              <label>App ID <input value={appId} onChange={(e) => setAppId(e.target.value)} /></label>
              ...
              <label>Client secret <input value={clientSecret} onChange={(e) => setClientSecret(e.target.value)} /></label>
              <label className="wide">Private key (PEM)
                <textarea rows={6} value={privateKey} onChange={(e) => setPrivateKey(e.target.value)} placeholder="-----BEGIN RSA PRIVATE KEY-----" />
              </label>
            </div>
            <div className="actions">
              <Button variant="primary" disabled={!appId.trim() || !privateKey.trim()} onClick={() => void saveManual()}>
              ...
              <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                <InputGroup>
                  ...
                  <InputGroup.Addon align="end">
                    <InputGroup.Button variant="secondary" disabled={!installationId.trim()} onClick={() => void completeInstall()}>
                      Verify
                    </InputGroup.Button>
                  </InputGroup.Addon>
                </InputGroup>
