...
export function DatabaseDataEngineer() {
  ...
  useAgentStart(({ log }) => {
    const object = migration.object || initialData?.object || (delivery.kind === 'signal' ? delivery.attributes?.object : undefined) || 'schema';
  ...
  useSubagent({
    ...
    description: 'Reviews a schema migration for data loss, irreversibility, and lock risk before it is applied.',
