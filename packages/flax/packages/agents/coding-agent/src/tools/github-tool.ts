...
const MUTATIONS = new Set(['createPullRequest','mergePullRequest','createBranch','commitFiles','createTag']);
...
export const githubTool = defineTool({
  ...
  input: v.object({
    method: v.picklist(['createPullRequest','getPullRequest','mergePullRequest','getIssue','createBranch','commitFiles','createTag','getDiff','getCiStatus']),
  ...
  async run({ data, step, log }) {
    ...
    const result = MUTATIONS.has(data.method)
      ? await step.do(`githubTool.${data.method}:${JSON.stringify(data.args ?? {})}`, () => rpc[data.method](data.args))
